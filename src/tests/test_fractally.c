...
static void fractally_test_basic(TCase *tc);

static const struct {
   const char *name;
   void (*build)(TCase *tc);
} tests[] = {
  { "basic", fractally_test_basic }
};

START_TEST(fractally_initialization)
{
}
END_TEST

static void
fractally_test_basic(TCase *tc)
{
   tcase_add_test(tc, fractally_initialization);
}
